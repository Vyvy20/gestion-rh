...
const employesResolvers = {
  ...
  Mutation: {
    addEmploye: async (
      parent,
      { prenom, nom, email, telephone, poste, salaire, password, jours },
      { user },
      info
    ) => {
      if (!user || user.role != 'rh') {
    ...
    updateEmploye: async (parent, { id, ...args }, { user }, info) => {
      // Assurez-vous que l'utilisateur est authentifié avant de procéder
      if (!user || (user.id !== id && user.role !== 'rh')) {
